"""Métricas de domínio do Climate Engine (Spec §5.1/§6).
...
_TRACKED = frozenset({TEMPERATURE_SHIFT, CLIMATE_THRESHOLD_CROSSED})


class ClimateMetricsSink:
    """Projeta os eventos do clima em contadores Prometheus."""
    ...
    def emit(self, event: DomainEvent) -> None:
        if event.event_type in _TRACKED:
            engine_domain_events.labels(engine="climate", event_type=event.event_type).inc()
