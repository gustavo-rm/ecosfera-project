"""Métricas de domínio do Hydrology Engine (Spec §5.1/§6).
...
_TRACKED = frozenset({ICE_SHEET_CHANGED, WATER_BALANCE_SHIFT})


class HydrologyMetricsSink:
    """Projeta os eventos do ciclo da água em contadores Prometheus."""
    ...
    def emit(self, event: DomainEvent) -> None:
        if event.event_type in _TRACKED:
            engine_domain_events.labels(engine="hydrology", event_type=event.event_type).inc()
