...
_bus = NullEventBus()
_planet_repo = InMemoryPlanetRepository()
...
def get_create_planet_use_case() -> CreatePlanetUseCase:
    return CreatePlanetUseCase(_planet_repo, get_simulation_params())


def get_run_tick_use_case() -> RunTickUseCase:
    return RunTickUseCase(_planet_repo, get_orchestrator(), get_explain_use_case())
